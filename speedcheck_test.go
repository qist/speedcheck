...
func TestProbeIPShortDeadlineContextStillProbesHTTP(t *testing.T) {
	l, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Fatalf("listen: %v", err)
	}
	...
	go func() {
		defer close(acceptDone)
		_ = l.(*net.TCPListener).SetDeadline(time.Now().Add(2 * time.Second))
		c, err := l.Accept()
		if err != nil {
			return
		}
	...
	p := newDefaultProber(800*time.Millisecond, false, nil, nil)
	ip := net.ParseIP("127.0.0.1").To4()
